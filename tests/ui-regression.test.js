...
test('the browser extension pane provides a direct download for the current package', () => {
  assert.match(app, /PromptHub-Extension-v3\.28\.0\.zip/);
  assert.match(app, /download="PromptHub-Extension-v3\.28\.0\.zip"/);
  assert.match(app, /下载浏览器插件/);
});

test('the extension visible version matches the packaged manifest version', () => {
  assert.match(fs.readFileSync(path.join(root, 'extension/manifest.json'), 'utf8'), /"version": "3\.28\.0"/);
  assert.match(popupHtml, /AI 提示词收集器 v3\.28\.0/);
});
