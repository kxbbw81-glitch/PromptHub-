(function (root, factory) {
  ...
  const CONTENT_TYPES = [
    ...
    { id: 'photography', label: '摄影与写实', test: item => item.category === '静物' || matches(item, /photography|photorealistic|realistic|camera|lens|摄影|写实|超写实|静物|photography & realism/) },
    ...
    { id: 'nature', label: '自然与生灵', test: item => ['风景', '自然', '动物'].includes(item.category) || matches(item, /landscape|scenery|wildlife|botanical|自然风景|野生动物|植物花卉/) },
    ...
    { id: 'video', label: '视频与动态', test: item => item.mediaType === 'video' || item.category === '视频提示词' || matches(item, /video prompt|video generation|motion|camera movement|视频提示词|文生视频|动态镜头/) },
  ...
  const STYLES = [
    ...
    { id: 'fantasy', label: '奇幻', test: item => matches(item, /fantasy|mythical|magic|奇幻|魔法|神话/) },
    { id: 'abstract', label: '抽象', test: item => matches(item, /abstract|surreal|抽象|超现实/) },
  ...
  const SCENES = [
    ...
    { id: 'city', label: '城市', test: item => item.category === '城市' || matches(item, /cityscape|urban|street scene|城市|街景|都市/) },
